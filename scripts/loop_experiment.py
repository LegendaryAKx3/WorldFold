...
from train_imagined_actor import behavior_clone, load_train, run_episode, save_actor
...
def load_many(dirs):
    ...
    return tuple(torch.cat([p[i] for p in parts]) for i in range(7))


def train_member(job):
    task_name, dirs, seed, steps, path = job
    task = TASKS[task_name]
    torch.set_num_threads(1)
    cur, prev, act, nxt = load_many(dirs)[:4]
...
def train_ensemble(task, dirs, round_dir, members, steps, seed, pool):
    paths = ensemble_paths(round_dir, members)
    jobs = [(task.name, dirs, seed * 100 + k, steps, p) for k, p in enumerate(paths) if not p.exists()]
    for done in pool.imap_unordered(train_member, jobs):
        print(f"trained {done}", flush=True)
    return [load_member(task, p) for p in paths]


def imagine(task, actor, ensemble, data, steps, seed, args, log, bc_coef=0.0, anchor_data=None):
    ...
    cur, prev, act, nxt, goal, anchors, stage = data
    batch = args.imagine_batch
    torch.manual_seed(seed)
    critic = Critic(feature_dim(task))
    trainer = ImaginationTrainer(ensemble[0], actor, critic, task, horizon=args.horizon, ensemble=ensemble,
    ...
    for step in range(1, steps + 1):
        ...
        if bc_coef > 0 and anchor_data is not None:
            a_cur, _, a_act, _, a_goal, _, a_stage = anchor_data
            j = torch.as_tensor(rng.integers(0, len(a_cur), batch))
            anchor = (a_cur[j], a_goal[j], a_stage[j], a_act[j])
        history.append(trainer.update(cur[idx], prev[idx], goal[idx], anchors[idx], stage[idx], anchor=anchor))
        if step % 500 == 0:
...
def evaluate(task, policies, seeds, pool, path):
    if path.exists():
        return json.loads(path.read_text())
    jobs = [(task.name, name, ckpt, seed, cond) for name, ckpt in policies
...
def collect_with_actor(task, checkpoint, out_dir, n, seed_base, pool):
    store = StateEpisodeStore(out_dir)
    if (out_dir / "manifest.json").exists():
        return
    manifest = []
    jobs = [(task.name, "actor", seed_base + i, None, str(checkpoint)) for i in range(n)]
...
def calibrate(task, args, pool):
    """Disagreement on real transitions vs on imagined rollouts with random actions."""
    round_dir = ROOT / args.output / "round_0"
    round_dir.mkdir(parents=True, exist_ok=True)
    ensemble = train_ensemble(task, [ROOT / args.base_data], round_dir, args.members, args.wm_steps, args.seed, pool)
    cur, prev, act = load_train(ROOT / args.base_data)[0][:3]
    idx = torch.as_tensor(np.random.default_rng(0).integers(0, len(cur), 2048))
    trainer = ImaginationTrainer(ensemble[0], Actor(feature_dim(task), task.action_dim),
                                 Critic(feature_dim(task)), task, horizon=10, ensemble=ensemble)
    with torch.no_grad():
        ...
        for _ in range(10):
            action = torch.rand(len(idx), task.action_dim) * 2 - 1
...
def run(task, args, pool):
    ...
    for r in range(args.rounds + 1):
        ...
        ensemble = train_ensemble(task, dirs, round_dir, args.members, args.wm_steps, args.seed + r, pool)
        data = load_many(dirs)
        ...
        if r == 0:
            ...
            if not init_path.exists():
                ...
                behavior_clone(task, actor, ensemble[0], clone_data, args.bc_steps, args.seed)
                save_actor(actor, ensemble[0], init_path)
            policies = [("expert", None)] + ([("ppo", None)] if task.name == "single" else []) + [(f"{ACTOR}_init", str(init_path))]
            print("baseline evaluation", flush=True)
            results["baselines"] = summarize(evaluate(task, policies, seeds, pool, round_dir / "eval_baselines.json"))
        ...
        if not path.exists():
            ...
            history = imagine(task, actor, ensemble, data, args.imagine_steps, args.seed + r, args, f"round_{r}",
        ...
        results[f"round_{r}"] = summarize(evaluate(task, [(ACTOR, str(path))], seeds, pool, round_dir / "eval.json"))
        results_path.write_text(json.dumps(results, indent=2))

        if r < args.rounds:
            target = out / f"collect_{r}"
            collect_with_actor(task, path, target, args.collect_per_round, args.collect_seed_base + r * 10000, pool)
